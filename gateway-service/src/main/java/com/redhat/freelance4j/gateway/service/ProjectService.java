...
public class ProjectService {
    ...
    public List<Project> getProjects() {
        final Response response = projectBaseService.request(MediaType.APPLICATION_JSON).get();
     ...
     */
    public List<Project> getProjectsByStatus(final String status) {
        final Response response = projectsGetByStatusService.resolveTemplate("theStatus", status)
    ...
    public Project getProject(final String projectId) {
        final Response response = projectGetByIdService.resolveTemplate("projectId", projectId)
                .request(MediaType.APPLICATION_JSON).get();
        if (response.getStatus() == 200) {
    ...
    @PostConstruct
    public void init() {
        projectBaseService = ((ResteasyClientBuilder) ClientBuilder.newBuilder()).connectionPoolSize(10).build()
